...
/*
    ...
    NOTE: Если память в 2047Mb адресовать на размер адреса (т.е. sizeof(void*)), то количество
          элементов листа достигнет значения 134152192, и если учесть что размер структуры
          хранящий один элемент составляет 24B, то для хранения списка максимально может
          потребоваться 3070Mb, который на секундочку адресует пространство в 2047Mb. Здесь
          размер структуры freelist не учитывается (т.е. sizeof(freelist)), по сравнению с тем
          что выше это смех и слезы =).

    NOTE: п.с. наихудший случай когда адресуется по 8 B, и свободные участки через одну,
          что сокращает размер хранения до 3070Mb, вместо ранее предложенных 6141Mb.
